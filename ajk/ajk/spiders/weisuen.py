...
class WeisuenSpider(scrapy.Spider):
    ...
    xzarea_codelist = ('fs', 'gz','zh','hui')
    xzarea_code = list(xzarea_codelist)

    def start_requests(self):
        start_urls = ['https://{}.fang.anjuke.com/'.format(code) for code in self.xzarea_code]
    ...
    def parse(self, response):
        for pn in range(1, 3):
            print("url", response.url + 'loupan/all/p{}/'.format(pn))
            url = response.url + 'loupan/all/p{}/'.format(pn)
